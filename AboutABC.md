...
* encrypt: AES in CTR mode; nonce is simple function of key

Authentication is built into the name, so there is no need for authenticating encryption modes. The nonce is effectively irrelevant for security, since each key is effectively used only once.
